...
<p class="mb-4 text-xs text-neutral-500">
  {data.options.length}
  env vars · {data.uniqueCount} unique to this variant · scraped
...
<div class="mb-4 flex flex-wrap items-center gap-3">
  ...
  <label class="flex items-center gap-1.5 text-sm text-neutral-700 dark:text-neutral-300">
    <input type="checkbox" bind:checked={uniqueOnly}>
    unique only
  </label>
</div>
  ...
  <table class="w-full border-collapse text-sm">
    ...
    <tbody>
      {#each filtered as o (o.name)}
        <tr
          class="border-b border-neutral-200 align-top dark:border-neutral-900"
          class:text-amber-600={o.unique}
          class:dark:text-amber-400={o.unique}
        >
          <td class="py-2 pr-4 font-mono text-sky-600 dark:text-sky-300">
            {o.name}
            {#if o.unique}
              <span
                class="ml-2 rounded bg-amber-400/15 px-1.5 py-0.5 text-[10px] font-medium text-amber-600 dark:text-amber-400"
                >unique</span
              >
            {/if}
          </td>
